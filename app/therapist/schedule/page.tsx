...
// Helper function to get sessions data using existing Supabase session
async function getTherapistSessions() {
  try {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3000'}/api/sessions/therapist/${session.user.id}`, {
      headers: {
...
function transformSessionToAppointment(session: any) {
  return {
    sid: session.sid,
    uid: session.uid,
    userName: session.users?.name || 'Unknown User',
    userImage: session.users?.profile_image || undefined,
    scheduled_date: session.scheduled_date,
    start_time: session.start_time,
    end_time: session.end_time,
    status: session.status === 'confirmed' ? 'approved' : session.status,
...
export default async function TherapistSchedulePage() {
  ...
  const sessionsData = await getTherapistSessions()
